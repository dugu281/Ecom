...
export default function SearchScreen() {
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `https://ecom-server.vercel.app/products/search?page=${page}&query=${query}&category=${category}&price=${price}&rating=${rating}&order=${order}`
        );
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const { data } = await axios.get(`https://ecom-server.vercel.app/products/categories`);
        setCategories(data);
      } catch (err) {
